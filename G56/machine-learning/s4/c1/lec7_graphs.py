...
def train_test_over_params(model, params, X_train, X_test, y_train, y_test):
    ...
    for i in values:
        ...
        tmp_train.append(mean_squared_error(
            y_train, tmp_model.predict(X_train)))
...
def plot_decision_function(model, df, x1, x2, y, colorbar=True):
    ...
    custom_colormap = LinearSegmentedColormap.from_list(
        'lista', color_palette_sequential)
    ...
    if model is DecisionTreeClassifier():
        ...
        plt.scatter(tmp_complete_mat[x1], tmp_complete_mat[x2],
                    alpha=.3, marker='.', color='slategrey')
    plt.xlabel(x1)
    plt.ylabel(x2)
